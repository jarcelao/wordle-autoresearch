...
class TestWordleEvaluator:
    ...
    def test_evaluate_train_multiple_games(self):
        ...
        side_effects = [
            "wrong", "wrong", "crane",  # game 1: win on 3rd
            "wrong", "wrong", "wrong", "wrong", "wrong", "wrong",  # game 2: loss
            "wrong", "beach",  # game 3: win on 2nd
        ]
...
class TestMain:
    ...
    def test_main_train_mode(self, mock_file, mock_evaluator, mock_stderr):
        ...
        with patch.object(sys, 'argv', ['harness', '--agent', 'simple', '--mode', 'train', '-o', 'results.json']):
            main()
    ...
    def test_main_test_mode(self, mock_file, mock_evaluator, mock_stderr):
        ...
        with patch.object(sys, 'argv', ['harness', '--agent', 'random', '--mode', 'test', '--games', '50', '-o', 'results.json']):
            main()
