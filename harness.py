...
from wordle import get_feedback, is_valid_guess, WORD_LENGTH, MAX_ATTEMPTS, fetch_wordle_word
...
class GameResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "guess_history": [
                {"guess": g.guess, "feedback": g.feedback}
...
class WordleEvaluator:
    ...
    def _run_evaluation(self, agent: Agent, words: List[str], mode: str) -> EvaluationResult:
...
def main():
    ...
    parser.add_argument("--agent", choices=list(AGENT_REGISTRY.keys()), default="simple",
                        help="Agent to evaluate")
    parser.add_argument("--mode", choices=["train", "test"], default="train",
                        help="Evaluation mode: train (fixed words) or test (API)")
    parser.add_argument("--games", type=int, default=100,
                        help="Number of games (for test mode)")
    parser.add_argument("--word-list", default="temp/train_words.txt",
                        help="Word list file (for train mode)")
    parser.add_argument("--cache", default="temp/api_cache",
                        help="Cache directory for API words")
    ...
    if args.mode == "train":
        if not Path(args.word_list).exists():
            default_words = [
                ...
                "kneel", "lemon", "melon", "noble", "ocean",
                "peace", "queen", "rider", "sunny", "table"
            ]
